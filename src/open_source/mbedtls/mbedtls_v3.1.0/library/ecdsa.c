 ...
 */
#if defined(MBEDTLS_INNER_ECDSA_VERIFY_RESTARTABLE_ALT)
                                     ...
                                     mbedtls_ecdsa_restart_ctx *rs_ctx )
{
    ...
    unsigned char r_buf[MAX_ECC_SIZE];
    ...
    if (klen == 0) {
        mbedtls_printf("get_curve_type failed\n");
        return MBEDTLS_ERR_ECP_BAD_INPUT_DATA;
    }

    ret = mbedtls_mpi_write_binary(r, r_buf, klen);
    if (ret != 0) {
        mbedtls_printf("mbedtls_mpi_write_binary failed\n");
        return ret;
    }

    ret = mbedtls_mpi_write_binary(s, s_buf, klen);
    if (ret != 0) {
        mbedtls_printf("mbedtls_mpi_write_binary failed\n");
        return ret;
    }

    ret = mbedtls_mpi_write_binary(&Q->MBEDTLS_PRIVATE(X), pub_x, klen);
    if (ret != 0) {
        mbedtls_printf("mbedtls_mpi_write_binary failed\n");
        return ret;
    }

    ret = mbedtls_mpi_write_binary(&Q->MBEDTLS_PRIVATE(Y), pub_y, klen);
    if (ret != 0) {
        mbedtls_printf("mbedtls_mpi_write_binary failed\n");
        return ret;
    }
