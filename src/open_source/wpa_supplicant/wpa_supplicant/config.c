				...
				const char *value)
{
	...
	if (ssid->key_mgmt == WPA_KEY_MGMT_WAPI_PSK) {
		ssid->hex_key_len = strlen(value) / 2; /* 2: half size of hex string to hexadecimal */
		if (hexstr2bin(value, ssid->psk, ssid->hex_key_len)) {
			ssid->hex_key_len = 0;
			return -1;
		}
	} else {
...
#if defined(CONFIG_WPS) || defined(LOS_CONFIG_WPA_ENTERPRISE)
...
#endif /* EXT_CODE_CROP */
#ifdef LOS_CONFIG_EAP_TLS
	{ STRe(private_key, cert.private_key) },
#endif /* LOS_CONFIG_EAP_TLS */
#ifndef EXT_CODE_CROP
...
#ifdef LOS_WPA_PATCH
	/* add default pmf setting for soc. */
...
#ifdef CONFIG_WPA3
	if (g_sta_opt_set.sae_pwe <= WIFI_SAE_PWE_UNSPECIFIED) {
		config->sae_pwe = WIFI_SAE_PWE_BOTH - 1;
	} else {
		config->sae_pwe = g_sta_opt_set.sae_pwe - 1;
	}
	if (g_sta_opt_set.sae_groups != NULL && g_sta_opt_set.sae_groups[0] != 0) {
		config->sae_groups = g_sta_opt_set.sae_groups;
	}
#endif /* CONFIG_WPA3 */
#endif /* LOS_WPA_PATCH */
