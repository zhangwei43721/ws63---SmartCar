...
__attribute__((weak)) int wpa_supplicant_ctrl_iface_ft_ds(
	struct wpa_supplicant *wpa_s, char *addr)
{
...
#ifndef LOS_WPS_REG_NON_PBC_CROP
#ifdef CONFIG_WPS_AP
	if (wpa_s->ap_iface) {
		int timeout = 0;
		char *pos;
...
#endif /* CONFIG_WPS_AP */
#endif /* LOS_WPS_REG_NON_PBC_CROP */
					    ...
					    char *buf, size_t buflen)
{
	...
	if (wpa_s == NULL || g_p2p_status_buf == NULL) {
		os_task_unlock(int_save);
		(void)os_event_write(g_p2p_event, WPA_EVENT_P2P_GET_STATUS_ERROR);
		return -1;
	}
	g_p2p_status_buf->mode =EXT_WIFI_P2P_DEVICE_ONLY;
	if (wpa_s->wpa_state >= WPA_ASSOCIATED) {
		...
		if (wpa_s->current_ssid->ssid_len)
			(void)os_memcpy(g_p2p_status_buf->ssid, wpa_s->current_ssid->ssid, wpa_s->current_ssid->ssid_len);

		(void)os_memcpy(g_p2p_status_buf->bssid, wpa_s->bssid, ETH_ALEN);
		g_p2p_status_buf->op_freq = wpa_s->assoc_freq;
		if (wpa_s->current_ssid->mode == WPAS_MODE_INFRA)
			g_p2p_status_buf->mode = EXT_WIFI_P2P_MODE_GC;
		else if (wpa_s->current_ssid->mode == WPAS_MODE_P2P_GO)
			g_p2p_status_buf->mode = EXT_WIFI_P2P_MODE_GO;
		else
			g_p2p_status_buf->mode = EXT_WIFI_P2P_DEVICE_ONLY;
	}

	if (wpa_s->wpa_state < WPA_AUTHENTICATING)
		g_p2p_status_buf->p2p_state = EXT_WIFI_P2P_DISCONNECTED;
	else if ((wpa_s->wpa_state >= WPA_AUTHENTICATING) && (wpa_s->wpa_state < WPA_COMPLETED))
		g_p2p_status_buf->p2p_state = EXT_WIFI_P2P_CONNECTING;
	else if (wpa_s->wpa_state == WPA_COMPLETED)
		g_p2p_status_buf->p2p_state = EXT_WIFI_P2P_CONNECTED;
	else
		fail = 1;
	g_p2p_status_buf = NULL;
	os_task_unlock(int_save);

	if (fail != 0)
		(void)os_event_write(g_p2p_event, WPA_EVENT_P2P_GET_STATUS_ERROR);
	else
		(void)os_event_write(g_p2p_event, WPA_EVENT_P2P_GET_STATUS_OK);
...
static int wpa_supplicant_ctrl_iface_list_networks(struct wpa_supplicant *wpa_s)
{
	...
	if ((wpa_s == NULL) || (wpa_s->conf == NULL) ||
	    (g_persistent_group_num == 0) || (g_p2p_persistent_group == NULL))
		return -1;

	g_persistent_group_num = WPA_MIN(g_persistent_group_num, P2P_MAX_PERSISTENT_CNT);
	ssid = wpa_s->conf->ssid;
	while (ssid) {
		/* 2: this network block includes parameters for a persistent P2P */
		if (ssid->disabled == 2) {
			g_p2p_persistent_group[i].network_id = ssid->id;
			os_memcpy(g_p2p_persistent_group[i].network_name, ssid->ssid, ssid->ssid_len);
			if (ssid->bssid_set)
				os_memcpy(g_p2p_persistent_group[i].go_bssid, ssid->bssid, ETH_ALEN);
			i++;
		}
		if ((unsigned int)i == g_persistent_group_num)
			return 0;

		ssid = ssid->next;
	}
	g_persistent_group_num = (unsigned int)i;
	g_p2p_persistent_group = NULL;
...
#ifdef LOS_WPA_PATCH
	wifi_dev = los_get_wifi_dev_by_name(wpa_s->ifname);
	if (wifi_dev == NULL)
		return -1;
#endif /* LOS_WPA_PATCH */
	...
	char *name, char *value)
{
	...
	if (os_strcmp(name, "ssid") == 0) {
		if (los_wpa_ssid_config_set(ssid, name, value) == EXT_WIFI_OK)
			goto SSID_SET;
		else
			return -1;
	}
			    ...
			    char *buf, size_t buflen)
{
	...
	new_pin = wpas_p2p_connect(wpa_s, addr, pin, wps_method,
				   ...
				   pd, he, group_ssid, group_ssid_len);
#endif
	if (new_pin == -2) {
...
static int p2p_ctrl_invite_persistent(struct wpa_supplicant *wpa_s, char *cmd)
{
	...
	return wpas_p2p_invite(wpa_s, _peer, ssid, NULL, freq, freq2, ht40, vht,
			       ...
			       allow_6ghz);
#else
	return wpas_p2p_invite(wpa_s, _peer, ssid, NULL, freq, pref_freq, he);
#endif
}
...
static int p2p_ctrl_group_add(struct wpa_supplicant *wpa_s, char *cmd)
{
	...
	return wpas_p2p_group_add(wpa_s, persistent, freq, he);
#endif /* EXT_CODE_CROP */
}
			 ...
			 char *buf, size_t buflen)
{
	...
	if (g_p2p_peers_discoverd == 0 || g_p2p_peers == NULL)
		return -1;

	g_p2p_peers_discoverd = WPA_MIN(g_p2p_peers_discoverd, P2P_SCAN_DEVICE_LIMIT);
	while (i < g_p2p_peers_discoverd) {
		info = p2p_get_peer_info(wpa_s->global->p2p, addr_ptr, next);
		if (info == NULL)
			break;

		(void)os_memcpy(g_p2p_peers[i].device_addr, info->p2p_device_addr, ETH_ALEN);
		...
		g_p2p_peers[i].group_capab = info->group_capab;

		(void)os_memcpy(addr, info->p2p_device_addr, ETH_ALEN);
		ssid = wpas_p2p_get_persistent(wpa_s, info->p2p_device_addr, NULL, 0);
		if (ssid)
			g_p2p_peers[i].persistent_id = ssid->id;
		else
			g_p2p_peers[i].persistent_id = -1;

		addr_ptr = addr;
		next = 1;
		i++;
	}
	g_p2p_peers_discoverd = i;
	g_p2p_peers = NULL;
...
#ifdef LOS_CONFIG_P2P
static int p2p_update_str(char **dst, const char *src, size_t len)
{
	if ((dst == NULL) || (src == NULL) || (len == 0))
		return -1;
	char *tmp = os_zalloc(len + 1);
	if (tmp == NULL) {
		wpa_error_log0(MSG_ERROR, "p2p_update_str: os_zalloc failed.");
		return -1;
	}
	os_memcpy(tmp, src, len);
	os_free(*dst);
	*dst = tmp;
	return 0;
}

static int p2p_ctrl_update_config(struct wpa_supplicant *wpa_s, const char *cmd)
{
	char *pos = NULL;
	char *cmd_walk = NULL;
	int size, len, i;
	...
	if ((wpa_s == NULL) || (wpa_s->conf == NULL) || (cmd == NULL))
		return -1;

	struct p2p_user_cfg_pos cfg_str[] = {
		{"devname=",    NULL, NULL, &wpa_s->conf->device_name,   CFG_CHANGED_DEVICE_NAME},
		{"manufact=",   NULL, NULL, &wpa_s->conf->manufacturer,  CFG_CHANGED_WPS_STRING},
		{"modname=",    NULL, NULL, &wpa_s->conf->model_name,    CFG_CHANGED_WPS_STRING},
		{"modnum=",     NULL, NULL, &wpa_s->conf->model_number,  CFG_CHANGED_WPS_STRING},
		{"sernum=",     NULL, NULL, &wpa_s->conf->serial_number, CFG_CHANGED_WPS_STRING},
		{"intent=",     NULL, NULL, NULL,                        0x0},
		{"opchan=",     NULL, NULL, NULL,                        0x0},
		{"listenchan=", NULL, NULL, NULL,                        0x0},
		{"wps_method=", NULL, NULL, NULL,                        0x0},
	};

	cmd_walk = (char *)cmd;
	size = sizeof(cfg_str) / sizeof(*cfg_str);
	/* Get every position from target string. */
	for (i = 0; i < size; ++i) {
		pos = os_strstr(cmd_walk, cfg_str[i].name);
		if (pos == NULL)
			continue;

		pos += os_strlen(cfg_str[i].name);

		if (os_strcmp(cfg_str[i].name, "intent=") == 0) {
			intent = atoi(pos);
			/* intent: 0-15 */
			if (intent < 0 || intent > 15)
				return -1;
		} else if (os_strcmp(cfg_str[i].name, "opchan=") == 0) {
			opchan = atoi(pos);
			/* ch: 1-13 */
			if (opchan < 1 || opchan >= 14)
				return -1;
		} else if (os_strcmp(cfg_str[i].name, "listenchan=") == 0) {
			listenchan = atoi(pos);
			/* ch: 1-13 */
			if (listenchan < 1 || listenchan >= 14)
				return -1;
		} else if (os_strcmp(cfg_str[i].name, "wps_method=") == 0) {
			wps_method = atoi(pos);
			/* wps_method: 0-1 */
			if (wps_method < 0 || wps_method > 1)
				return -1;
		} else {
			len = atoi(pos);
			/* 64: manufacturer has the maximum length. */
			if (len < 0 || len > 64)
				return -1;
			pos = os_strchr(pos, ',');
			if (pos == NULL)
			...
			cfg_str[i].pos_start = pos + 1;
			cfg_str[i].pos_end = cfg_str[i].pos_start + len;
			pos = cfg_str[i].pos_end + 1;
		}

		cmd_walk = pos;
	}

	/* Filter the valid character string and update config. */
	for (i = 0; i < size; ++i) {
		if (cfg_str[i].pos_start == NULL || cfg_str[i].pos_end == NULL || cfg_str[i].dst == NULL ||
			(cfg_str[i].pos_end <= cfg_str[i].pos_start))
			continue;
		(void)p2p_update_str(cfg_str[i].dst, cfg_str[i].pos_start, cfg_str[i].pos_end - cfg_str[i].pos_start);
		wpa_s->conf->changed_parameters |= cfg_str[i].changed;
	}

	if (intent != -1)
		wpa_s->conf->p2p_go_intent = intent;

	if (opchan != -1) {
		/* 81: operating class */
		wpa_s->conf->p2p_oper_reg_class = 81;
		wpa_s->conf->p2p_oper_channel = opchan;
	...
	if (listenchan != -1) {
		...
		wpa_s->conf->p2p_listen_channel = listenchan;
		wpa_s->conf->changed_parameters |= CFG_CHANGED_P2P_LISTEN_CHANNEL;
	}

	if (wps_method != -1) {
		if (wps_method == 0) {
			os_memcpy(wpa_s->conf->config_methods, "push_button", sizeof("push_button"));
		} else if (wps_method == 1) {
			os_memcpy(wpa_s->conf->config_methods, "display", sizeof("display"));
		}
...
#ifndef LOS_CONFIG_EXT_DRIVER_NOT_SUPPORT
...
#endif /* LOS_CONFIG_EXT_DRIVER_NOT_SUPPORT */
#endif /* CONFIG_P2P */
			   ...
			   char *reply, int reply_size, int *reply_len)
{
	...
	if (wpa_s->wpa_state == WPA_INTERFACE_DISABLED) {
		*reply_len = -1;
		(void)os_event_write(g_wpa_event, WPA_EVENT_SCAN_OK);
		return;
	}
#else
	(void)reply;
...
static int wpa_sta_status(struct wpa_supplicant *wpa_s)
{
	if ((g_sta_status == NULL) || (wpa_s == NULL)) {
		(void)os_event_write(g_wpa_event, WPA_EVENT_STA_STATUS_FAIL);
		return EXT_WIFI_FAIL;
	}

	if (wpa_s->wpa_state >= WPA_ASSOCIATED) {
		struct wpa_ssid *ssid = wpa_s->current_ssid;
		os_memcpy(g_sta_status->bssid, wpa_s->bssid, ETH_ALEN);

		if (los_freq_to_channel(wpa_s->assoc_freq, &g_sta_status->channel) != EXT_WIFI_OK) {
			(void)os_event_write(g_wpa_event, WPA_EVENT_STA_STATUS_FAIL);
			return EXT_WIFI_FAIL;
		}
		if ((ssid != NULL) && (ssid->ssid_len > 0) && (ssid->ssid != NULL)) {
			os_memcpy(g_sta_status->ssid, ssid->ssid, ssid->ssid_len);
        } else if ((wpa_s->current_bss != NULL) && (wpa_s->current_bss->ssid_len > 0) &&
            (wpa_s->current_bss->ssid != NULL)) {
            os_memcpy(g_sta_status->ssid, wpa_s->current_bss->ssid, wpa_s->current_bss->ssid_len);
        }
	}
	if (wpa_s->wpa_state < WPA_AUTHENTICATING) {
		g_sta_status->status = EXT_WIFI_DISCONNECTED;
	} else if (wpa_s->wpa_state >= WPA_AUTHENTICATING && wpa_s->wpa_state < WPA_COMPLETED) {
		g_sta_status->status = EXT_WIFI_CONNECTING;
	} else if (wpa_s->wpa_state == WPA_COMPLETED) {
		g_sta_status->status = EXT_WIFI_CONNECTED;
	} else {
		(void)os_event_write(g_wpa_event, WPA_EVENT_STA_STATUS_FAIL);
		return EXT_WIFI_FAIL;
	}

	(void)os_event_write(g_wpa_event, WPA_EVENT_STA_STATUS_OK);
	return EXT_WIFI_OK;
}

static int wpa_get_sta_status(struct wpa_supplicant *wpa_s, void *addr)
{
    ext_wifi_status_sem *connect_status = (ext_wifi_status_sem *)addr;

    if ((connect_status == NULL) || (wpa_s == NULL)) {
        connect_status->status = WIFI_STASTUS_PTR_NULL;
        osal_sem_up(&connect_status->sta_status_sem);
        return EXT_WIFI_FAIL;
	}

	if (wpa_s->wpa_state >= WPA_ASSOCIATED) {
		struct wpa_ssid *ssid = wpa_s->current_ssid;
		os_memcpy(connect_status->conn_status->bssid, wpa_s->bssid, ETH_ALEN);

		if (los_freq_to_channel(wpa_s->assoc_freq, &connect_status->conn_status->channel) != EXT_WIFI_OK) {
            connect_status->status = WIFI_STASTUS_CHAN_FAIL;
            osal_sem_up(&connect_status->sta_status_sem);
			return EXT_WIFI_FAIL;
		}
		if ((ssid != NULL) && (ssid->ssid_len > 0) && (ssid->ssid != NULL)) {
			os_memcpy(connect_status->conn_status->ssid, ssid->ssid, ssid->ssid_len);
        } else if ((wpa_s->current_bss != NULL) && (wpa_s->current_bss->ssid_len > 0) &&
            (wpa_s->current_bss->ssid != NULL)) {
            os_memcpy(connect_status->conn_status->ssid, wpa_s->current_bss->ssid, wpa_s->current_bss->ssid_len);
        }
	}
	if (wpa_s->wpa_state < WPA_AUTHENTICATING) {
		connect_status->conn_status->status = EXT_WIFI_DISCONNECTED;
	} else if (wpa_s->wpa_state >= WPA_AUTHENTICATING && wpa_s->wpa_state < WPA_COMPLETED) {
		connect_status->conn_status->status = EXT_WIFI_CONNECTING;
	} else if (wpa_s->wpa_state == WPA_COMPLETED) {
		connect_status->conn_status->status = EXT_WIFI_CONNECTED;
	} else {
        connect_status->status = WIFI_STASTUS_SET_FAIL;
        osal_sem_up(&connect_status->sta_status_sem);
		return EXT_WIFI_FAIL;
	}

    connect_status->status = WIFI_STASTUS_OK;
    osal_sem_up(&connect_status->sta_status_sem);
	return EXT_WIFI_OK;
}
...
static int hapd_deauth_peer(struct hostapd_data *hapd, const char *addr_txt)
{
	...
	(void)os_event_write(g_softap_event, WPA_EVENT_AP_DEAUTH_OK);
	return EXT_WIFI_OK;
OUT:
	(void)os_event_write(g_softap_event, WPA_EVENT_AP_DEAUTH_FAIL);
	return EXT_WIFI_FAIL;
}
...
static int wpa_supplicant_iface_remove(struct wpa_supplicant *wpa_s)
{
	...
	if ((wpa_s == NULL) || (wpa_s->global == NULL)) {
		...
		(void)os_event_write(g_wpa_event, WPA_EVENT_REMOVE_IFACE_FLAG);
		return EXT_WIFI_FAIL;
	}
	wpa_s->reassociate = 0;
	wpa_s->disconnected = 1;
...
static int wpa_supplicant_show_sta_check(ext_wifi_iftype iftype)
{
	if ((iftype != EXT_WIFI_IFTYPE_AP) && (iftype != EXT_WIFI_IFTYPE_MESH_POINT) &&
		(iftype != EXT_WIFI_IFTYPE_P2P_GO))
		return -1;

	if (g_hapd == NULL)
		return -1;

	if ((iftype == EXT_WIFI_IFTYPE_AP) && (g_ap_sta_info == NULL))
		return -1;

#ifdef LOS_CONFIG_MESH
	if (g_mesh_get_sta_flag && (g_mesh_sta_info == NULL))
		return -1;
#endif /* LOS_CONFIG_MESH */

#ifdef LOS_CONFIG_P2P
	if ((iftype == EXT_WIFI_IFTYPE_P2P_GO) && (g_p2p_client_info == NULL))
		return -1;
#endif /* #ifdef LOS_CONFIG_P2P */
	return 0;
}


static void wpa_get_ap_info(ext_wifi_iftype iftype, struct sta_info *sta, unsigned int *sta_count)
{
	if ((sta == NULL) || (sta_count == NULL) ||(g_ap_sta_info == NULL))
		return;
	if ((iftype == EXT_WIFI_IFTYPE_AP) && (sta->flags & WLAN_STA_AUTHORIZED)) {
...
static void wpa_get_mesh_info(ext_wifi_iftype iftype, struct sta_info *sta, unsigned int *sta_count)
{
	if ((sta == NULL) || (sta_count == NULL) ||(g_mesh_sta_info == NULL))
		return;
...
#ifdef LOS_CONFIG_P2P
static void wpa_get_p2p_info(ext_wifi_iftype iftype, struct sta_info *sta, unsigned int *client_count)
{
	struct p2p_message msg;
	const u8 *p2p_dev_addr = NULL;
	if ((sta == NULL) || (client_count == NULL) ||(g_p2p_client_info == NULL))
		return;

	if ((iftype == EXT_WIFI_IFTYPE_P2P_GO) && (sta->flags & WLAN_STA_AUTHORIZED)) {
		(void)os_memcpy(g_p2p_client_info->mac, sta->addr, ETH_ALEN);

		if (sta->p2p_ie == NULL)
			return;
		os_memset(&msg, 0, sizeof(msg));
		if (p2p_parse_p2p_ie(sta->p2p_ie, &msg))
			return;

		(void)os_memcpy(g_p2p_client_info->device_name, msg.device_name, os_strlen(msg.device_name) + 1);

		p2p_dev_addr = p2p_get_go_dev_addr(sta->p2p_ie);
		if (p2p_dev_addr == NULL)
			return;

		(void)os_memcpy(g_p2p_client_info->dev_addr, p2p_dev_addr, ETH_ALEN);
...
static int wpa_supplicant_show_sta(ext_wifi_iftype iftype)
{
	struct sta_info *sta = NULL;
	unsigned int sta_count = 0;

	g_sta_num = WPA_MIN(g_sta_num, WIFI_DEFAULT_MAX_NUM_STA);
	if (wpa_supplicant_show_sta_check(iftype) < 0) {
		return -1;
	}
	for (sta = g_hapd->sta_list; sta != NULL; sta = sta->next) {
		...
		wpa_get_ap_info(iftype, sta, &sta_count);
#ifdef LOS_CONFIG_MESH
		wpa_get_mesh_info(iftype, sta, &sta_count);
#endif /* LOS_CONFIG_MESH */
#ifdef LOS_CONFIG_P2P
		wpa_get_p2p_info(iftype, sta, &sta_count);
#endif /* LOS_CONFIG_P2P */
	}
	g_sta_num = sta_count;
	g_ap_sta_info = NULL;
...
#ifdef LOS_WPA_PATCH
static int wpa_supplicant_ctrl_iface_add(struct wpa_supplicant *wpa_s, const char *ifname)
{
	...
	if ((wpa_s == NULL) || (wifi_dev == NULL) ||
		...
		(void)os_event_write(g_wpa_event, WPA_EVENT_ADD_IFACE_ERROR);
		return EXT_WIFI_FAIL;
	}
	...
	(void)os_event_write(g_wpa_event, WPA_EVENT_ADD_IFACE_OK);
	return EXT_WIFI_OK;
}
#endif /* LOS_WPA_PATCH */
					 ...
					 char *buf, size_t *resp_len)
{
	...
	wifi_dev = los_get_wifi_dev_by_priv(wpa_s);
	if (wifi_dev == NULL) {
	...
	if (os_strncmp(buf, "AP_DEAUTH ", 10) == 0) {
		if (wifi_dev->iftype == EXT_WIFI_IFTYPE_AP)
			(void)hapd_deauth_peer((struct hostapd_data *)(void *)wpa_s, buf + 10);
		goto OUT;
	} else if (os_strcmp(buf, "SHOW_STA") == 0) {
		if (wpa_supplicant_show_sta(wifi_dev->iftype) < 0)
			(void)os_event_write(g_softap_event, WPA_EVENT_AP_SHOW_STA_ERROR);
		else
			(void)os_event_write(g_softap_event, WPA_EVENT_AP_SHOW_STA_OK);
		goto OUT;
#ifdef LOS_CONFIG_P2P
	} else if (os_strcmp(buf, "LIST_NETWORKS") == 0) {
		if (wpa_supplicant_ctrl_iface_list_networks(wpa_s) < 0)
			(void)os_event_write(g_p2p_event, WPA_EVENT_P2P_GET_PERSISTENT_GROUP_ERROR);
		else
			(void)os_event_write(g_p2p_event, WPA_EVENT_P2P_GET_PERSISTENT_GROUP_OK);
		goto OUT;
#endif /* LOS_CONFIG_P2P */
	}

	if ((wpa_s == NULL) || (wifi_dev->iftype == EXT_WIFI_IFTYPE_AP) || (eloop_is_running(ELOOP_TASK_WPA) != EXT_WIFI_OK))
		goto OUT;

#ifdef CONFIG_MESH
	if (os_strcmp(buf, "JOIN_MESH") == 0) {
		if (wpa_s->ifmsh != NULL)
			los_wpa_join_mesh(wpa_s);
		goto OUT;
	}
#endif
	...
	if (os_strncmp(buf, "STATUS", 6) == 0) {
		...
		reply_len = p2p_ctrl_peer(wpa_s, buf + 9, reply,
					      reply_size);
		(void)os_event_write(g_p2p_event, WPA_EVENT_P2P_GET_PEER_OK);
#ifndef LOS_P2P_CROP
...
#endif /* LOS_CONFIG_EXT_DRIVER_NOT_SUPPORT */
#ifdef LOS_CONFIG_P2P
	} else if (os_strncmp(buf, "P2P_UPDATE_CONFIG ", 18) == 0) {
		if (p2p_ctrl_update_config(wpa_s, buf + 18) < 0)
			reply_len = -1;
#endif /* LOS_CONFIG_P2P */
#endif /* CONFIG_P2P */
#ifdef CONFIG_WIFI_DISPLAY
	...
	} else if (os_strncmp(buf, WPA_CTRL_RSP, os_strlen(WPA_CTRL_RSP)) == 0)
	{
        ...
        (void)wpa_sta_status(wpa_s);
    } else if (os_strcmp(buf, "STA_STATUS") > 0) {
        void *addr = (void *)atoi((char *)(buf + 11));
        (void)wpa_get_sta_status(wpa_s, addr);
#ifdef LOS_CONFIG_MESH
	} else if (os_strncmp(buf, "MESH_DEAUTH ", 12) == 0) {
		(void)wpa_mesh_deauth_peer(wpa_s, buf + 12);
	} else if (os_strncmp(buf, "MESH_SET_ACCEPT ", 16) == 0) {
		(void)wpa_mesh_set_accept(wpa_s, buf + 16);
#endif /* LOS_CONFIG_MESH */
#endif /* LOS_WPA_PATCH */
		...
		if (reply == NULL) {
			reply = os_malloc(reply_size);
			if (reply == NULL) {
				osal_printk("scan_results malloc fail.\r\n");
				wpa_error_log0(MSG_ERROR, "ctrl_iface_process fail to malloc buffer.");
				*resp_len = 1;
				return NULL;
			}
		...
		g_result_len = (size_t)reply_len;
		(void)os_event_write(g_wpa_event, WPA_EVENT_GET_SCAN_RESULT_OK);
#endif /* LOS_WPA_PATCH */
#ifndef EXT_CODE_CROP
...
#ifdef LOS_WPA_PATCH
        osal_printk("write event WPA_EVENT_SCAN_RESULT_CLEAR_OK.\r\n");
        (void)os_event_write(g_wpa_event, WPA_EVENT_SCAN_RESULT_CLEAR_OK);
#endif /* LOS_WPA_PATCH */
#ifdef CONFIG_TDLS
		...
		if (wpas_ctrl_iface_wnm_bss_query(wpa_s, buf + 14))
				reply_len = -1;
		(void)os_event_write(g_wpa_event, WPA_EVENT_STA_WNM_BSS_QUERY);
	} else if (os_strncmp(buf, "COLOC_INTF_REPORT ", 18) == 0) {
		if (wpas_ctrl_iface_coloc_intf_report(wpa_s, buf + 18))
			reply_len = -1;
	} else if (os_strncmp(buf, "WNM_Notify ", 11) == 0) {
		if (wpas_mbo_update_non_pref_chan(wpa_s, buf + 11))
			reply_len = -1;
		(void)os_event_write(g_wpa_event, WPA_EVENT_STA_WNM_NOTIFY);
#endif /* CONFIG_WNM */
#ifndef EXT_CODE_CROP
