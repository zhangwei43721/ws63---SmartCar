...
#ifdef LOS_WPA_PATCH
/* Identifies that this is a scan request triggered by remove_network cli */
static bool wpa_scan_triggered_by_rm_net_cli_flag = false;

void wpa_set_scan_triggered_by_rm_net_cli_flag(bool flag)
{
	wpa_scan_triggered_by_rm_net_cli_flag = flag;
}

bool wpa_get_scan_triggered_by_rm_net_cli_flag(void)
{
	return wpa_scan_triggered_by_rm_net_cli_flag;
}
#endif
			      ...
			      enum wpa_states state)
{
	...
	if ((wpa_is_sta(wpa_s) == WPA_FLAG_ON) && (ssid_temp != NULL) && (state == WPA_COMPLETED)) {
#ifdef CONFIG_SSID_RECONNECT
		(void)los_remove_bss_with_same_ssid(wpa_s);
#endif
	...
	if (state == WPA_COMPLETED && wpa_s->new_connection) {
		...
		if (((wpa_is_sta(wpa_s) == WPA_FLAG_ON) && (g_sta_delay_report_flag == WPA_FLAG_OFF)) ||
		    (wifi_dev->iftype == EXT_WIFI_IFTYPE_P2P_CLIENT)) {
#ifdef CONFIG_SSID_RECONNECT
			(void)los_remove_bss_with_same_ssid(wpa_s);
#endif
			osal_printk("+NOTICE:CONNECTED\r\n");
		}
		 ...
		 */
		if ((wpa_is_sta(wpa_s) == WPA_FLAG_ON) && (g_connecting_ssid != NULL) &&
		    (g_connecting_ssid->mode == WPAS_MODE_INFRA) && (g_fast_connect_flag == WPA_FLAG_ON))
			g_connecting_ssid->frequency = 0;
#ifdef LOS_WPA_PATCH
		event_wifi_connected connect = {0};
		if ((g_wpa_event_cb != NULL) && (wpa_s->ifmsh == NULL) && (ssid != NULL) &&
			(g_sta_delay_report_flag == WPA_FLAG_OFF)) {

			if (ssid->ssid != NULL) {
				os_memcpy(connect.ssid, ssid->ssid, ssid->ssid_len);
			}
			os_memcpy(connect.bssid, wpa_s->bssid, EXT_WIFI_MAC_LEN);
			os_memcpy(connect.ifname, wpa_s->ifname, WIFI_IFNAME_MAX_SIZE);
			connect.ssid_len = ssid->ssid_len;
			wpa_comm_event_report(EXT_WIFI_EVT_CONNECTED, wpa_s, &connect);
		} else {
...
#endif /* CONFIG_FILS && IEEE8021X_EAPOL */
	if ((state == WPA_COMPLETED) && (wpa_is_sta(wpa_s) == WPA_FLAG_ON)) {
		if (g_fast_connect_flag == WPA_FLAG_ON)
			(void)os_event_write(g_wpa_event, WPA_EVENT_QUICK_CONNECT_OK);
	}
}
			      ...
			      u8 *wpa_ie, size_t *wpa_ie_len)
{
	...
	if (bss_rsn && (ssid->proto & WPA_PROTO_RSN) &&
...
#endif /* CONFIG_HS20 */
#ifdef CONFIG_WAPI
	} else if (((ssid->proto & WPA_PROTO_WAPI) || (ssid->key_mgmt & WPA_KEY_MGMT_WAPI_PSK)) &&
	           bss && wpa_bss_get_ie(bss, WLAN_EID_WAPI)) {
		wpa_dbg(wpa_s, MSG_DEBUG, "WAPI: input proto:%x, key_mgmt:%x, group:%x, pairwise:%x",
			ssid->proto, ssid->key_mgmt, ssid->group_cipher, ssid->pairwise_cipher);
		...
		ie.pairwise_cipher = WPA_CIPHER_SMS4;
		ie.group_cipher = WPA_CIPHER_SMS4;
		ie.key_mgmt = WPA_KEY_MGMT_WAPI_PSK;
...
#ifdef CONFIG_WAPI
	if ((proto != WPA_PROTO_WAPI) && wpa_sm_set_assoc_wpa_ie_default(wpa_s->wpa, wpa_ie, wpa_ie_len))
#else
...
__attribute__((weak)) int wpas_valid_bss(struct wpa_supplicant *wpa_s,
...
#else
	if (hostapd_set_freq_params(&vht_freq, mode->mode, freq->freq,
					freq->channel, freq->ht_enabled) != 0)
		return;
#endif /* LOS_WPA_PATCH */
	...
	enum wpa_drv_update_connect_params_mask *mask)
{
	...
	if (ssid->key_mgmt & (WPA_KEY_MGMT_WAPI_PSK)) {
		wpa_dbg(wpa_s, MSG_INFO, "wapi associate, ignore this extended capabilities ie.");
	} else {
#endif /* CONFIG_WAPI */
...
#else
		if (rsn && wpa_parse_wpa_ie(rsn, 2 + rsn[1], &ie) == 0) {
			if ((unsigned int)ie.capabilities & WPA_CAPABILITY_MFPC) {
#ifndef EXT_CODE_CROP
...
#ifdef CONFIG_WAPI
	if (ssid->key_mgmt & WPA_KEY_MGMT_WAPI_PSK) {
		if (wapi_asue_event_assoc(wpa_s, bss, ssid, &params) != 0) {
			/* 15: deauth reason code. Authentication rejected because of wai failure */
					    ...
					    const u8 *addr)
{
	...
	old_ssid = wpa_s->current_ssid;
#if CONFIG_SSID_RECONNECT
	if ((wpa_is_sta(wpa_s) == WPA_FLAG_OFF) || (g_ssid_reconn_max_count <= 1))
#endif
...
#ifdef CONFIG_WAPI
	if (wpa_s->key_mgmt == WPA_KEY_MGMT_WAPI_PSK) {
		wapi_asue_event_disassoc(wpa_s, reason_code);
	}
#endif /* CONFIG_WAPI */
...
int wpa_supplicant_remove_network(struct wpa_supplicant *wpa_s, int id)
{
	...
	if (!ssid) {
		if (g_wpa_rm_network == SOC_WPA_RM_NETWORK_START)
			(void)os_event_write(g_wpa_event, WPA_EVENT_STA_RM_NETWORK_OK);
		return -1;
	}
#ifdef LOS_CONFIG_P2P
	p2p_persistent = ssid->disabled == 2 ? 1 : 0;
#endif /* LOS_CONFIG_P2P */

	if (wpa_is_sta(wpa_s) == WPA_FLAG_ON) {
		if (g_reconnect_set.pending_flag) {
...
#ifdef LOS_WPA_PATCH
	if ((g_wpa_rm_network == SOC_WPA_RM_NETWORK_START) && (wpa_s->wpa_state <= WPA_SCANNING))
		wpa_set_scan_triggered_by_rm_net_cli_flag(true);
#endif /* LOS_WPA_PATCH */
	return 0;
}
...
#endif /* EXT_CODE_CROP */
		if ((wpa_is_sta(wpa_s) == WPA_FLAG_ON) && (g_fast_connect_flag == WPA_FLAG_ON)) {
			wpa_supplicant_req_scan(wpa_s, 0, 0);
...
#else
	/* l2_packet_init should not be called again. */
...
#ifdef LOS_CONFIG_P2P
u8 device_names[]  = {"xxxx_p2p"};
u8 manufacturers[] = {"xxxx"};
u8 model_names[]   = {"xxxx"};
u8 model_numbers[] = {"xxxx"};
u8 mydevice_type[] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
u8 serial_number[] = {"000000000000000"};
				     ...
				     const struct wpa_interface *iface)
{
...
#else
#ifdef LOS_CONFIG_P2P
	if (strstr(iface->ifname, "p2p") != NULL)
	...
	if (wpa_supplicant_init_wpa(wpa_s) < 0)
		return -1;
#ifdef LOS_CONFIG_P2P
	if (wpa_s->conf->config_methods != NULL) {
		os_free(wpa_s->conf->config_methods);
		wpa_s->conf->config_methods = NULL;
	}
	...
	if (wpa_s->global->p2p == NULL) {
		if (wpa_s->conf->device_name == NULL)
			wpa_s->conf->device_name = os_strdup((const char *)device_names);
		if (wpa_s->conf->manufacturer == NULL)
			wpa_s->conf->manufacturer = os_strdup((const char *)manufacturers);
		if (wpa_s->conf->model_name == NULL)
			wpa_s->conf->model_name = os_strdup((const char *)model_names);
		if (wpa_s->conf->model_number == NULL)
			wpa_s->conf->model_number = os_strdup((const char *)model_numbers);
		if (wpa_s->conf->serial_number == NULL)
			wpa_s->conf->serial_number = os_strdup((const char *)serial_number);
	}
					...
					int notify, int terminate)
{
	...
	if (wifi_dev != NULL && wifi_dev->iftype == EXT_WIFI_IFTYPE_STATION) {
		int rst = wapi_deinit_iface(wpa_s);
...
#endif /* EXT_CODE_CROP */
#ifdef LOS_WPA_PATCH
	/* to be make sure if p2p GO remove before p2p device */
#if defined(LOS_CONFIG_P2P) && defined(LOS_CONFIG_P2P_TWO_IFACE)
	if (wifi_dev != NULL && wifi_dev->iftype == EXT_WIFI_IFTYPE_P2P_DEVICE)
		los_free_p2p_wifi_dev();
	else
#endif /* LOS_CONFIG_P2P  && LOS_CONFIG_P2P_TWO_IFACE */
...
struct wpa_global * wpa_supplicant_init(void)
{
	...
	ret = eap_register_methods();
	if (ret) {
		wpa_error_log0(MSG_ERROR, "Failed to register methods");
		return NULL;
	}
...
void wpa_supplicant_deinit(struct wpa_global *global)
{
	...
	os_free(global->p2p_disallow_freq.range);
	os_free(global->p2p_go_avoid_freq.range);
	if (global->add_psk != NULL)
		(void)memset_s(global->add_psk, sizeof(*(global->add_psk)), 0, sizeof(*(global->add_psk)));
	os_free(global->add_psk);
	global->p2p_disallow_freq.range = NULL;
	global->p2p_go_avoid_freq.range = NULL;
	global->add_psk = NULL;
#endif /* CONFIG_P2P */
...
#ifdef CONFIG_SSID_RECONNECT
int los_add_bss_with_same_ssid(const struct wpa_supplicant *wpa_s, const struct wpa_ssid *group, int only_first_ssid)
{
	if (wpa_is_sta(wpa_s) == WPA_FLAG_OFF || group->mode != WPAS_MODE_INFRA)
		return 0;
	if (g_conf_aps_bssid == NULL) {
		int idx = 0;
		for (int i = 0; i < wpa_s->last_scan_res_used; i++) {
			struct wpa_bss *bss = wpa_s->last_scan_res[i];
			if (bss->ssid_len == group->ssid_len &&
			    os_memcmp(bss->ssid, group->ssid, bss->ssid_len) == 0) {
				g_ssid_reconn_max_count++;
			}
		}
		g_conf_aps_bssid = (struct wpa_bss **)os_malloc(sizeof(struct wpa_bss *) * (size_t)g_ssid_reconn_max_count);
		if (g_conf_aps_bssid == NULL)
			return -1;
		for (int i = 0; i < wpa_s->last_scan_res_used; i++) {
			struct wpa_bss *bss = wpa_s->last_scan_res[i];
...
int los_remove_bss_with_same_ssid(const struct wpa_supplicant *wpa_s)
{
	if (wpa_is_sta(wpa_s) == WPA_FLAG_OFF)
		return 0;

	os_free(g_conf_aps_bssid);
	g_conf_aps_bssid = NULL;
	g_ssid_reconn_count = 1;
	g_ssid_reconn_max_count = 0;
...
void wpas_request_disconnection(struct wpa_supplicant *wpa_s)
{
	...
	wpa_s->disconnected = 1;
#ifdef LOS_WPA_PATCH
	if ((wpa_is_sta(wpa_s) == WPA_FLAG_ON) && (g_reconnect_set.pending_flag == WPA_FLAG_ON)) {
		osal_printk("wpas_request_disconnection cancel reconnect_timeout !\r\n");
		(void)eloop_cancel_timeout(wpa_supplicant_reconnect_timeout, wpa_s, NULL);
		g_reconnect_set.pending_flag = WPA_FLAG_OFF;
	}
