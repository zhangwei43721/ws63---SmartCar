...
#define CONFIG_SERVER_IP            "10.227.77.198"                     // 要连接的服务器IP
...
void *Wifi_Task(void *arg)
{
    ...
    while(1)
    {
        ...
        result =  sendto(sock_fd, body_buf, sizeof(body_buf), 0,(struct sockaddr *)&send_addr, addr_length);
        if (result) 
        {
            printf("result: %d, sendData:%s\r\n", result, body_buf);
        }
...
void *Sensor_Body_Task(void *arg)
{
    unused(arg);
    bool body_stat;
    ...
    while(1)
    {
        body_stat = getBodyState();
        // 字符串合并
        sprintf((char *)body_buf, "Body:%d", body_stat);
        printf("%s\n",body_buf);
        osal_msleep(100);
        // 人体红外状态复位
        //setBodayState(false);
...
void sensor_body_task_create(void)
{
    osal_task *task_handle = NULL;
    
    osal_kthread_lock();

    task_handle = osal_kthread_create((osal_kthread_handler)Sensor_Body_Task, 0, "BodyTask", BODY_TASK_STACK_SIZE);
    if (task_handle != NULL)
    {
        osal_kthread_set_priority(task_handle, BODY_TASK_PRIO);
    }
...
{
    printf("Hi3863开发板--WIFI UDP 发送温湿度，人体状态数据实验\n");
