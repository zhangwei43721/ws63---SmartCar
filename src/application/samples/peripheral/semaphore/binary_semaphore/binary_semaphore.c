...
static int thread_task1(const char *arg)
{
    ...
    while(1)
    {
        ...
        // 请求信号量-1
        osal_sem_down_timeout(&semID, OSAL_WAIT_FOREVER);
...
void binary_semaphore_entry(void)
{
    ...
    // 创建信号量
    int ret = osal_sem_binary_sem_init(&semID, 1);
    if(ret == OSAL_FAILURE)
    {
        osal_printk("create binary sem failed\r\n");
        return ;
    }
    osal_printk("ID = %d, create binary sem OK\r\n", semID);
