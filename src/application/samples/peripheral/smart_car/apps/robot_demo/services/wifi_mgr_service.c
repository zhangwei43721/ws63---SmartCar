...
static void on_start(void) {
  ...
  if (ssid[0] == '\0') {
    ...
    sta_task_stop();
    if (ap_task_start()) s_mgr_state = MGR_STATE_AP;
  } else {
    printf("[WiFi Mgr] 配置 SSID=%s，启动 STA\r\n", ssid);
    ap_task_stop();
    s_sta_fail_cnt = 0;
    bsp_wifi_set_current_config(ssid, pwd);
    if (sta_task_start()) s_mgr_state = MGR_STATE_STA;
  }
}
...
static void on_sta_fail(void) {
  if (s_mgr_state != MGR_STATE_STA) return;
  s_sta_fail_cnt++;
...
static int mgr_task_main(void* arg) {
  ...
  while (1) {
    ...
    switch (msg.id) {
      case WIFI_MSG_START:        on_start();      break;
      case WIFI_MSG_STA_GOT_IP:   on_sta_got_ip(); break;
      case WIFI_MSG_STA_FAIL:     on_sta_fail();   break;
      case WIFI_MSG_STA_STOPPED:                     break;
      case WIFI_MSG_AP_READY:     on_ap_ready();   break;
      case WIFI_MSG_AP_STOPPED:                      break;
      default:                                       break;
    }
  }
  return 0;
...
int bsp_wifi_mgr_init(void) {
  ...
  if (osal_msg_queue_create("wifi_mgr", 8, &s_msg_queue, 0,
                            sizeof(bsp_wifi_msg_t)) != OSAL_SUCCESS) {
    printf("[WiFi Mgr] 队列创建失败\r\n");
    return -1;
  }

  osal_kthread_lock();
  s_mgr_task = osal_kthread_create((osal_kthread_handler)mgr_task_main, NULL,
                                    "wifi_mgr", MGR_STACK_SIZE);
  if (s_mgr_task) osal_kthread_set_priority(s_mgr_task, MGR_PRIO);
  osal_kthread_unlock();
...
int bsp_wifi_smart_init(void) {
  if (bsp_wifi_mgr_init() != 0) return -1;
