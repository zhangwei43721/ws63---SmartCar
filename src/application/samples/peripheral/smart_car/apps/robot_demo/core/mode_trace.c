...
static void trace_tick_once(void)
{
    ...
    if (++debug_cnt >= 20) {
        debug_cnt = 0;
        printf("TRACE: L=%d M=%d R=%d, ADC: L=%d M=%d R=%d mV\n", left, middle, right, tcrt5000_get_left_adc(),
               tcrt5000_get_middle_adc(), tcrt5000_get_right_adc());
    }
...
void mode_trace_enter(void)
{
    ...
    osal_kthread_lock();
    g_trace_task =
        osal_kthread_create((osal_kthread_handler)trace_task_entry, NULL, "trace_task", TRACE_TASK_STACK_SIZE);
    if (g_trace_task != NULL) {
