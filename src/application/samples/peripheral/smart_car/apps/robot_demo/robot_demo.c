...
static void *mode_switch_isr(const char *arg)
{
    ...
    if (current_status == CAR_STOP_STATUS) {
        ...
        printf("Switch to WIFI CONTROL mode\n");
        robot_set_status(CAR_WIFI_CONTROL_STATUS);
#elif defined(CONFIG_SMART_CAR_BT_SPP_ENABLE) || defined(CONFIG_SMART_CAR_BT_SPP_EXAMPLE)
        printf("Switch to BLUETOOTH CONTROL mode\n");
        robot_set_status(CAR_BT_CONTROL_STATUS);
#else
        printf("Switch to STOP mode\n");
        robot_set_status(CAR_STOP_STATUS);
#endif
    } else if (current_status == CAR_WIFI_CONTROL_STATUS) {
        ...
        robot_set_status(CAR_STOP_STATUS);
#endif
    } else if (current_status == CAR_BT_CONTROL_STATUS) {
        printf("Switch to STOP mode\n");
        robot_set_status(CAR_STOP_STATUS);
    }
...
#if defined(CONFIG_SMART_CAR_WIFI_ENABLE) || defined(CONFIG_SMART_CAR_WIFI_EXAMPLE)
    printf("  3rd press: WIFI CONTROL mode\n");
#if defined(CONFIG_SMART_CAR_BT_SPP_ENABLE) || defined(CONFIG_SMART_CAR_BT_SPP_EXAMPLE)
    printf("  4th press: BLUETOOTH CONTROL mode\n");
    printf("  5th press: STOP mode\n");
#else
    printf("  4th press: STOP mode\n");
#endif
#elif defined(CONFIG_SMART_CAR_BT_SPP_ENABLE) || defined(CONFIG_SMART_CAR_BT_SPP_EXAMPLE)
    printf("  3rd press: BLUETOOTH CONTROL mode\n");
    printf("  4th press: STOP mode\n");
#else
    ...
    while (1) {
        ...
        switch (status) {
            ...
            case CAR_WIFI_CONTROL_STATUS:
                printf("Entering WIFI CONTROL mode\n");
                robot_wifi_control_mode();
                printf("Exiting WIFI CONTROL mode\n");
                break;

            case CAR_BT_CONTROL_STATUS:
                printf("Entering BLUETOOTH CONTROL mode\n");
                robot_bt_control_mode();
                printf("Exiting BLUETOOTH CONTROL mode\n");
                break;
