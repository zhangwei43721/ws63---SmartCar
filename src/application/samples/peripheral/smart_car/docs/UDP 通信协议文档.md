...
typedef struct __attribute__((packed)) {
    uint8_t type;     // 数据包类型
    uint8_t cmd;      // 命令/模式编号
    int8_t  data1;    // 方向命令时为速度幅值 / 参数高字节
    int8_t  data2;    // 方向命令时保留 / 参数低字节
    int8_t  ext;      // 扩展字段/保留
} udp_packet_t;
```
...
| ----------- | ------- | ----- | ------------------------- |
| 0           | `type`  | uint8 | **0x01**                  |
| 1           | `cmd`   | uint8 | **方向命令**（0停 1前进 2后退 3左转 4右转，与 `CarDriveCmd` 对齐） |
| 2           | `data1` | int8  | **速度幅值**（1~100，当前由固件固定） |
| 3           | `data2` | int8  | 保留 (0x00)               |
| 4           | `ext`   | uint8 | 保留 (0x00)               |

> 前端/语音只上报方向，具体转向动作（差速 or 舵机）由嵌入式按底盘类型翻译。
