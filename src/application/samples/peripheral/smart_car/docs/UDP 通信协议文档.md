...
**核心逻辑**：选定小车 IP 后，手机必须每 1 秒主动向小车发送一次心跳包。小车收到第一个心跳包后，将停止广播，并锁定手机 IP 进入"连接状态"。
...
> **注意**：如果手机超过 5 秒未发送心跳，小车会自动释放当前连接，重新回到"扫描阶段"开始广播。
...
### 5.3 状态回传 (小车 → 手机, Type=0x02)

连接成功后，小车每 2s 或在传感器突变时向手机发送。手机监听 `8889` 接收。
...
* 用户点击"连接" → 开启定时器。
* **定时器任务**：每 1000ms 向小车 IP:8888 发送 `0xFE` 字节数组。
* **状态反馈**：当手机开始收到小车发回的 `0x02` 包时，说明握手成功。
...
* **接收**：解析 `0x02` 数据包，使用 `runOnUiThread` 更新界面上的距离和红外灯状态。
