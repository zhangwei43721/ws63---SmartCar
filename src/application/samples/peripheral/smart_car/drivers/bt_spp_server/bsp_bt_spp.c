...
static uint8_t g_device_name[NAME_MAX_LENGTH] = {
    'W', 'S', '6', '3', '_', 'U', 'A', 'R', 'T'}; /* 蓝牙设备名称 */
...
static bsp_bt_spp_status_t g_bt_spp_status =
    BSP_BT_SPP_STATUS_IDLE; /* 当前连接状态 */
...
static uint8_t u16_low_u8(uint16_t val) { return (uint8_t)(val & 0xff); }

static uint8_t u16_high_u8(uint16_t val) {
  return (uint8_t)((val >> 8) & 0xff);
}

static uint8_t ble_set_adv_flag_data(uint8_t* set_adv_data_position,
                                     uint8_t max_len) {
  ...
  if (memcpy_s(set_adv_data_position, max_len, &adv_flags, BLE_ADV_FLAG_LEN) !=
      EOK) {
    return 0;
  }
...
static uint8_t ble_set_adv_appearance(uint8_t* set_adv_data_position,
                                      uint8_t max_len) {
  ble_appearance_st adv_appearance_data = {
      .length = BLE_ADV_APPEARANCE_LENGTH - 1,
      .adv_data_type = BLE_ADV_APPEARANCE_DATA_TYPE,
      .catogory_id = {u16_low_u8(BLE_ADV_CATEGORY_KEYBOARD_VALUE),
                      u16_high_u8(BLE_ADV_CATEGORY_KEYBOARD_VALUE)},
  };
  if (memcpy_s(set_adv_data_position, max_len, &adv_appearance_data,
               BLE_ADV_APPEARANCE_LENGTH) != EOK) {
    return 0;
  }
  return BLE_ADV_APPEARANCE_LENGTH;
}

static uint16_t bsp_bt_spp_set_adv_data(uint8_t* set_adv_data,
                                        uint8_t adv_data_max_len) {
  uint8_t idx = 0;
...
static uint16_t ble_set_scan_response_data(uint8_t* scan_rsp_data,
                                           uint8_t scan_rsp_data_max_len) {
  uint8_t idx = 0;
  ...
  if (memcpy_s(scan_rsp_data, scan_rsp_data_max_len, &tx_power_level,
               sizeof(ble_tx_power_level_st)) != EOK) {
    return 0;
  }
  ...
  if (memcpy_s(&scan_rsp_data[idx], scan_rsp_data_max_len - idx, g_device_name,
               sizeof(g_device_name)) != EOK) {
    return 0;
  }
...
static uint8_t bsp_bt_spp_config_adv(void) {
  ...
  adv_data_len =
      bsp_bt_spp_set_adv_data(set_adv_data, EXT_ADV_OR_SCAN_RSP_DATA_LEN);
  if ((adv_data_len > EXT_ADV_OR_SCAN_RSP_DATA_LEN) || (adv_data_len == 0)) {
    return 1;
  }

  scan_rsp_data_len = ble_set_scan_response_data(set_scan_rsp_data,
                                                 EXT_ADV_OR_SCAN_RSP_DATA_LEN);
  if ((scan_rsp_data_len > EXT_ADV_OR_SCAN_RSP_DATA_LEN) ||
      (scan_rsp_data_len == 0)) {
    return 1;
  }
 ...
 * @brief 添加CCCD描述符
 */
static void bsp_bt_spp_add_ccc_descriptor(uint32_t server_id,
                                          uint32_t srvc_handle) {
 ...
 * @brief 添加特征和CCCD
 */
static void bsp_bt_spp_add_characteristic(uint32_t server_id,
                                          uint32_t srvc_handle) {
 ...
 * @brief 服务添加回调
 */
static void bsp_bt_spp_service_add_cbk(uint8_t server_id, bt_uuid_t* uuid,
 ...
 * @brief 特征添加回调 - 记录特征句柄
 */
static void bsp_bt_spp_char_add_cbk(uint8_t server_id, bt_uuid_t* uuid,
 ...
 */
static void bsp_bt_spp_desc_add_cbk(uint8_t server_id, bt_uuid_t* uuid,
 ...
 * @brief 服务启动回调
 */
static void bsp_bt_spp_service_start_cbk(uint8_t server_id, uint16_t handle,
                                         errcode_t status) {
  UNUSED(server_id);
  UNUSED(handle);
  UNUSED(status);
 ...
 */
static void bsp_bt_spp_write_req_cbk(uint8_t server_id, uint16_t conn_id,
                                     gatts_req_write_cb_t* req,
                                     errcode_t status) {
  UNUSED(server_id);
  UNUSED(conn_id);
  UNUSED(status);
 ...
 * @brief 读请求回调
 */
static void bsp_bt_spp_read_req_cbk(uint8_t server_id, uint16_t conn_id,
                                    gatts_req_read_cb_t* req,
                                    errcode_t status) {
  UNUSED(server_id);
  UNUSED(conn_id);
  UNUSED(req);
 ...
 * @brief MTU改变回调
 */
static void bsp_bt_spp_mtu_changed_cbk(uint8_t server_id, uint16_t conn_id,
 ...
 * @brief 连接状态改变回调
 */
static void bsp_bt_spp_connect_change_cbk(uint16_t conn_id, bd_addr_t* addr,
 ...
 * @brief 配对结果回调
 */
static void bsp_bt_spp_pair_result_cbk(uint16_t conn_id, const bd_addr_t* addr,
                                       errcode_t status) {
  UNUSED(conn_id);
  UNUSED(addr);
  UNUSED(status);
 ...
 * @brief 连接参数更新回调
 */
static void bsp_bt_spp_conn_param_update_cbk(
...
static errcode_t bsp_bt_spp_register_server(void) {
  ...
  if (memcpy_s(uuid.uuid, uuid.uuid_len, g_app_uuid, sizeof(g_app_uuid)) !=
      EOK) {
    return ERRCODE_BT_FAIL;
  }
 ...
 * @return SPP状态
 */
bsp_bt_spp_status_t bsp_bt_spp_get_status(void) { return g_bt_spp_status; }
