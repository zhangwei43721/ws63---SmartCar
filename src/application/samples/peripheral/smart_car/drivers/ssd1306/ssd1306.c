...
bool ssd1306_Init(void) {
  ...
  uint32_t probe_ret = uapi_i2c_master_write(CONFIG_I2C_MASTER_BUS_ID,
                                             I2C_SLAVE2_ADDR, &probe_data);
  if (probe_ret != 0) return false;  // 设备探测失败，OLED 不存在或通信失败
...
void ssd1306_UpdateScreen(void) {
  ...
  memcpy_s(&data[count], SSD1306_BUFFER_SIZE + 1, SSD1306_Buffer,
           SSD1306_BUFFER_SIZE);
...
uint8_t ssd1306_GetDisplayOn(void) { return SSD1306.DisplayOn; }
