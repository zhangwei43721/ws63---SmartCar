...
void bsp_ssd1306_draw_string16(uint8_t x, uint8_t y, const char *str, BspSsd1306Color color)
{
    ...
    while (*p) {
        ...
        if (len == 1) {
            bsp_ssd1306_set_cursor(curr_x, curr_y + 3);
            bsp_ssd1306_draw_char(*p, Font_7x10, color);
            curr_x += 7;
        } else {
            ...
            if (index >= 0) {
                ...
                for (int i = 0; i < 16; i++) {
                    ...
                    for (int b = 0; b < 8; b++) {
                        bsp_ssd1306_draw_pixel(curr_x + i, curr_y + b,
                                               (byte & (1 << b)) ? color : (BspSsd1306Color)!color);
                    }
                    byte = bitmap[i + 16];
                    for (int b = 0; b < 8; b++) {
                        bsp_ssd1306_draw_pixel(curr_x + i, curr_y + 8 + b,
                                               (byte & (1 << b)) ? color : (BspSsd1306Color)!color);
                    }
                }
                curr_x += 16;
