...
/** @brief 获取当前 WiFi 状态 */
bsp_wifi_status_t bsp_wifi_get_status(void) { return s_status; }

/** @brief 获取当前 WiFi 模式（STA/AP） */
bsp_wifi_mode_t   bsp_wifi_get_mode(void)   { return s_mode; }

/** @brief 设置 WiFi 状态 */
void bsp_wifi_set_status(bsp_wifi_status_t st) { s_status = st; }

/** @brief 设置 WiFi 模式 */
void bsp_wifi_set_mode(bsp_wifi_mode_t m)      { s_mode = m; }

/** @brief 获取本机 IP 地址字符串 */
int bsp_wifi_get_ip(char* ip_str, uint32_t len) {
  if (!ip_str || len < 16) return -1;
...
void bsp_wifi_notify_event(bsp_wifi_event_t event) {
  ...
  if (s_sync_inited &&
      (event == BSP_WIFI_EVENT_STA_GOT_IP || event == BSP_WIFI_EVENT_STA_FAIL)) {
...
void bsp_wifi_set_current_config(const char* ssid, const char* pwd) {
  if (ssid) strncpy_s(s_ssid, sizeof(s_ssid), ssid, sizeof(s_ssid) - 1);
  if (pwd)  strncpy_s(s_pwd,  sizeof(s_pwd),  pwd,  sizeof(s_pwd)  - 1);
}

/** @brief 获取当前配置的 WiFi SSID 和密码 */
void bsp_wifi_get_current_config(char* ssid, uint32_t ssid_len,
                                  char* pwd, uint32_t pwd_len) {
  if (ssid && ssid_len > 0) {
...
int bsp_wifi_scan_list(bsp_wifi_scan_item_t* items, uint32_t max_count,
                       uint32_t* out_count) {
  if (!items || !out_count || !max_count) return -1;
  *out_count = 0;

  bool was_on = (wifi_is_sta_enabled() == 1);
  if (!was_on && wifi_sta_enable() != ERRCODE_SUCC) return -1;

  uint32_t num = 32;
  wifi_scan_info_stru* res = osal_kmalloc(sizeof(wifi_scan_info_stru) * num, OSAL_GFP_ATOMIC);
  if (!res) { if (!was_on) wifi_sta_disable(); return -1; }

  if (wifi_sta_scan() != ERRCODE_SUCC) {
    osal_kfree(res);
    if (!was_on) wifi_sta_disable();
    return -1;
  }
  ...
  if (!was_on) wifi_sta_disable();
  if (ret != ERRCODE_SUCC) { osal_kfree(res); return -1; }

  uint32_t idx = 0;
  for (uint32_t i = 0; i < num && idx < max_count; i++) {
    if (res[i].ssid[0] == 0) continue;
...
/** @brief 设置 STA 任务退出标志 */
void sta_set_should_exit(bool v) { s_should_exit = v; }

/** @brief 获取 STA 任务退出标志 */
bool sta_get_should_exit(void)   { return s_should_exit; }

/** @brief 唤醒 STA 任务（用于断线后重连） */
void sta_task_wakeup(void)       { osal_sem_up(&s_wait_sem); }

/** @brief 启动 STA（WiFi 客户端）任务 */
bool sta_task_start(void) {
  if (s_sta_task) return true;
  s_should_exit = false;
  osal_kthread_lock();
  s_sta_task = osal_kthread_create((osal_kthread_handler)sta_task_main, NULL,
                                    "wifi_sta", 4096);
  if (s_sta_task) osal_kthread_set_priority(s_sta_task, 24);
  osal_kthread_unlock();
  if (!s_sta_task) { printf("[STA] task 创建失败\r\n"); return false; }
  printf("[STA] task 启动\r\n");
  return true;
}
...
void sta_task_stop(void) {
  ...
  int to = 0;
  while (s_sta_task && to < 100) { osal_msleep(10); to++; }
  s_sta_task = NULL;
}
...
int sta_task_main(void* arg) {
  ...
  while (!s_should_exit) {
    ...
    /* 扫描 */
    if (wifi_sta_scan() != ERRCODE_SUCC) { osal_msleep(5000); continue; }
    if (osal_sem_down_timeout(&s_conn_sem, 5000) != OSAL_SUCCESS) { osal_msleep(5000); continue; }

    uint32_t num = 32;
    wifi_scan_info_stru* res = osal_kmalloc(sizeof(wifi_scan_info_stru) * num, OSAL_GFP_ATOMIC);
    if (!res) { osal_msleep(5000); continue; }
    if (wifi_sta_get_scan_info(res, &num) != ERRCODE_SUCC) { osal_kfree(res); osal_msleep(5000); continue; }
