...
int bsp_wifi_scan_list(bsp_wifi_scan_item_t *items, uint32_t max_count, uint32_t *out_count)
{
    ...
    *out_count = 0;
    ...
    for (uint32_t i = 0; i < num && idx < max_count; i++) {
        if (s_scan_buf[i].ssid[0] == 0) continue;
...
static int sta_task_main(void *arg)
{
    ...
    if (wifi_sta_enable() != ERRCODE_SUCC) {
        ...
        if (s_exit_sem_inited) osal_sem_up(&s_exit_sem);
        return 0;
    }
    ...
    while (!s_should_exit) {
        ...
        for (int scan_retry = 0; scan_retry < 2; scan_retry++) {
            if (wifi_sta_scan() != ERRCODE_SUCC) goto connect_fail;
            if (osal_sem_down_timeout(&s_conn_sem, 5000) != OSAL_SUCCESS) goto connect_fail;
            num = 32;
            if (wifi_sta_get_scan_info(s_conn_scan_buf, &num) != ERRCODE_SUCC) goto connect_fail;
            if (num > 0) break;
        ...
        if (wifi_sta_connect(&cfg) != ERRCODE_SUCC) goto connect_fail;
